        ...
        return `
            <div class="product-card bg-white rounded-3xl p-5 shadow-lg">
                <div class="product-info mb-4">
                    <h3 class="product-title text-lg font-bold text-gray-900 mb-3 text-center">${product.name}</h3>

                    <div class="product-image-container bg-white rounded-xl flex items-center justify-center overflow-hidden mb-2">
                        <img src="${product.image}" alt="${product.name}" class="product-image">
                    </div>
                    <p class="product-description text-sm text-gray-600 mb-3">${product.description}</p>
                    ${installmentText}
                    <p class="product-price text-2xl text-center font-black text-blue-900 mt-1">R$ ${product.price.toFixed(2).replace('.', ',')}</p>
                </div>
                <a href="https://wa.me/558596945408?text=Olá!%20Gostaria%20de%20comprar%20o%20${encodeURIComponent(product.name)}" target="_blank" class="buy-button block text-center bg-gradient-to-r from-blue-500 to-blue-600 text-white py-2 px-4 rounded-xl font-bold text-sm hover:from-green-600 hover:to-green-700 transition-all shadow-lg hover:shadow-xl">
                    Comprar agora
                </a>
            </div>
        `;
